...
export default function LegalModalContent() {
  ...
  return (
    <div className="flex flex-col space-y-6 text-earth-forest">
      
      {/* Tab Selector */}
      <div className="flex flex-wrap items-center gap-1.5 p-1.5 bg-canvas-raised rounded-2xl border border-earth-forest/10 text-xs">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl font-medium transition-all text-xs ${
                isActive
                  ? 'bg-earth-forest text-canvas shadow-xs font-bold'
                  : 'text-sandstone-muted hover:text-earth-forest hover:bg-canvas-subtle'
              }`}
            >
              <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-earth-sage' : ''}`} />
              <span>{tab.label}</span>
            </button>
          );
        })}
      ...
      {activeTab === 'privacy' && (
        <div className="space-y-6 text-xs leading-relaxed text-sandstone-muted text-left">
          ...
          <div className="p-4 rounded-2xl bg-earth-sageSoft border border-earth-sage/30 text-earth-forest space-y-1.5">
            <h4 className="font-serif font-bold text-sm flex items-center gap-2 text-earth-forest">
              <ShieldCheck className="w-4 h-4 text-earth-sage" /> 100% Local Data Sovereignty Guarantee
            </h4>
          ...
          <div className="space-y-3">
            <h5 className="font-serif font-bold text-sm text-earth-forest">1. Zero Cloud Uploads & Zero Remote Storage</h5>
            <p>
              All transaction records, account balances, categorization rules, and encrypted receipt scans reside exclusively in your device's local SQLite database protected by hardware-backed AES-256 encryption (<code className="font-mono text-earth-forest">sqflite_sqlcipher</code>). There are no user accounts, no cloud databases, and no remote backups.
            </p>
          </div>

          <div className="space-y-3">
            <h5 className="font-serif font-bold text-sm text-earth-forest">2. Zero SMS Permission Policy</h5>
            <p>
              Unlike traditional expense trackers, VITT <strong>NEVER requests or uses the <code className="font-mono text-earth-forest font-semibold">READ_SMS</code> or <code className="font-mono text-earth-forest font-semibold">RECEIVE_SMS</code> Android permissions</strong>. VITT has zero technical capability to read your personal text messages, one-time passwords (OTPs), personal conversations, or contacts.
            </p>
          </div>

          <div className="space-y-3">
            <h5 className="font-serif font-bold text-sm text-earth-forest">3. On-Device Notification Listener Consent</h5>
            <p>
              Automatic UPI expense tracking relies exclusively on Android's standard <code className="font-mono text-earth-forest">NotificationListenerService</code>. On fresh installation, Notification Access is disabled by default and requires explicit system permission granted by you. You may revoke this access at any time from Android System Settings.
            </p>
          </div>

          <div className="space-y-3">
            <h5 className="font-serif font-bold text-sm text-earth-forest">4. DPDP Act 2023 Compliance & Data Deletion Rights</h5>
            <p>
      ...
      {activeTab === 'terms' && (
        <div className="space-y-6 text-xs leading-relaxed text-sandstone-muted text-left">
          ...
          <div className="space-y-3">
            <h5 className="font-serif font-bold text-sm text-earth-forest">1. Nature of the Application</h5>
            <p>
          ...
          <div className="space-y-3">
            <h5 className="font-serif font-bold text-sm text-earth-forest">2. Independent Solo Developer Project</h5>
            <p>
          ...
          <div className="space-y-3">
            <h5 className="font-serif font-bold text-sm text-earth-forest">3. Non-Commercial & Personal Household Scope</h5>
            <p>
          ...
          <div className="space-y-3">
            <h5 className="font-serif font-bold text-sm text-earth-forest">4. Limitation of Liability</h5>
            <p>
      ...
      {activeTab === 'sebi' && (
        <div className="space-y-6 text-xs leading-relaxed text-sandstone-muted text-left">
          ...
          <div className="p-4 rounded-2xl bg-earth-terracottaSoft border border-earth-terracotta/30 text-earth-terracotta space-y-2">
            <div className="flex items-center gap-2 font-bold font-serif text-sm">
              <AlertTriangle className="w-4 h-4 shrink-0 text-earth-terracotta" />
              <span>SEBI Investment Advisers & Research Analysts Disclaimer</span>
            </div>
            <p className="font-serif italic text-earth-forest text-[11px] leading-relaxed">
          ...
          <div className="space-y-3">
            <h5 className="font-serif font-bold text-sm text-earth-forest">Educational & Analytical Scope Only</h5>
            <p>
      ...
      {activeTab === 'grievance' && (
        <div className="space-y-6 text-xs leading-relaxed text-sandstone-muted text-left">
          ...
          <div className="p-4 rounded-2xl bg-canvas-raised border border-earth-forest/10 space-y-3">
            <h5 className="font-serif font-bold text-earth-forest flex items-center gap-2 text-sm">
              <Mail className="w-4 h-4 text-earth-sage" /> DPDP Act 2023 Grievance Officer
            </h5>
          ...
          <div className="space-y-3">
            <h5 className="font-serif font-bold text-sm text-earth-forest">Statutory Grievance Resolution SLA</h5>
            <p>
          ...
          <div className="p-3.5 rounded-xl bg-canvas-raised border border-earth-forest/10 flex items-center justify-between">
            <span className="font-mono text-[11px] text-earth-forest">GitHub Issue Tracker</span>
            <a 
              href="https://github.com/nikhil49023/VITT/issues"
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs font-bold text-earth-sage flex items-center gap-1 hover:underline"
            >
